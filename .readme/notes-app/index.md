...
**Stack:** Next.js 16, React, Plate.js, Supabase, Dexie, TanStack Query, Zustand, Tailwind
...
| [Todo Block](features/todo-block.md) | Checkbox/task list block type for the editor |
...
| [Fantasy Theme](features/fantasy-theme.md) | Fantasy-styled UI skin |
| [Sci-Fi Theme](features/sci-fi-theme.md) | Sci-fi styled UI skin |
| Archive Releases Names | Archiving a type/tag should release its slug/name for reuse |
...
| [Content Filtering](features/content-filtering.md) | Extend advanced filters to search entry body content |
| [Manual Entry Ordering](features/manual-entry-ordering.md) | Drag-and-drop reordering of entries in sidebar and type pages |
| [Apply Theme from Themes Page](features/apply-theme-from-themes-page.md) | Activate button on account-level custom themes page |
| Editor Slash Menu Emoji | Emoji item in editor slash menu |
| Quick Create Modal UI Polish | Restyled heading as uppercase section label with muted background |
| Quick Create Keyboard Indicators | Keyboard hint footer in quick create modal |
| [Example Campaign](features/example-campaign.md) | Pre-populated pirate campaign for guest mode |

## Planned Features

| Feature | Description |
|---------|-------------|
| Signup Bot Protection | Add CAPTCHA (Cloudflare Turnstile) to signup form |
| Self-Host Supabase | Migrate from Supabase Cloud to self-hosted on Hetzner VPS |

## Deferred

| Feature | Description | Reason |
|---------|-------------|--------|
| Optimistic Locking | Prevent silent overwrites during simultaneous metadata edits | Low practical risk; Yjs handles editor content. Revisit if pain point. |
