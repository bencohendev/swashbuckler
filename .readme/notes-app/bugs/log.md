...
| Date | Bug | Fix |
|------|-----|-----|
| 2026-03-06 | Editor flashes loading screen when sidebar finishes loading | Solo→Collaborative editor switch: before space loaded, `canEdit` defaulted to false, so editor rendered Solo content immediately. When space + shares loaded, `isCollaborative` flipped, switching to CollaborativeEditor with its own `isSynced=false` overlay. Fixed by gating on space availability + shares resolution before rendering in Supabase mode, plus server-side type hydration to eliminate staggered loads |
...
| 2026-03-05 | Sidebar & types page empty state flash | Restored `typesLoading` to sidebar skeleton gate; removed forced empty array in TypeTableView during loading, replaced with skeleton rows |
